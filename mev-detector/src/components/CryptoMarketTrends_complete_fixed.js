      ...
      <div className="space-y-6">
                ...
                {defiData.map((protocol, index) => (
                  <tr key={index} className="border-b border-slate-800/50 hover:bg-slate-700/30">
                    <td className="py-4">
                      <div className="flex items-center">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-neon/20 to-green-500/20 flex items-center justify-center mr-3">
                          <span className="text-xs font-bold text-brand-neon">{protocol.protocol?.charAt(0) || 'D'}</span>
                        </div>
                    ...
                    <td className="text-right py-4 text-slate-300">#{protocol.rank}</td>
                    <td className="text-right py-4 text-green-400">{protocol.yield}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
            ...
            <BarChart data={defiData.map(item => ({ 
              label: item.protocol, 
              value: parseFloat(item.tvl.replace(/[^0-9.-]+/g,"")) 
            }))} height={150} />
          </div>
          
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
            <h3 className="text-xl font-bold text-white mb-4">Yield Opportunities</h3>
            <BarChart data={defiData.map(item => ({ 
              label: item.protocol, 
              value: parseFloat(item.yield.replace('%','')) 
            }))} height={150} />
          </div>
        </div>
      </div>
